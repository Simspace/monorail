...
var _StyledTreeRowAlignmentBox =
/*#__PURE__*/
(0, _styledComponents.default)(TreeRowAlignmentBox).withConfig({
  ...
  function flattenWithDepthInner(depth, ancestors, index) {
    return currentTree => {
      return [{
        ...
        index
      }, ...(0, _pipeable.pipe)(currentTree.forest, forest => forest.map((childTree, idx) => [childTree, idx]), (0, _Array.chain)(([childTree, childIdx]) => {
        return (0, _pipeable.pipe)(childTree, flattenWithDepthInner(depth + 1, ancestors.concat({
...
const TreeRowToggleAndDepthLine = props => {
  ...
  return _react.default.createElement(_react.default.Fragment, null, Array(depth).fill(depth).map((_, idx) => _react.default.createElement(TreeRowAlignmentBox, {
    key: idx
  }, _react.default.createElement(TreeRowDepthLine, null))), _react.default.createElement(_StyledTreeRowAlignmentBox, {
    _css: cssToggle
  }, !isLeaf && _react.default.createElement(TreeRowToggleContainer, null, _react.default.createElement(_IconButton.IconButton, {
    icon: isOpen ? 'arrow_drop_down' : 'arrow_right',
    display: _buttonTypes.ButtonDisplay.Chromeless,
    onClick: onClick
  }), showDepthLine && _react.default.createElement(TreeRowDepthLine, {
    isVisible: isOpen
  }))));
};
...
const useTreeList = props => {
  ...
  const rows = (0, _pipeable.pipe)(forest, f => f.map((tr, idx) => [tr, idx]), (0, _Array.chain)(([data, index]) => flattenWithDepth(data, a => getTreeNodeKey(a), index))).filter(row => {
    return (0, _Array.intersection)(_Eq.eqString)(openRows, row.ancestors.map(a => a.key)).length === row.ancestors.length;
  });
  return {
    rows,
    toggleNode,
