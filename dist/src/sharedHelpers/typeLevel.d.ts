 ...
 */
export declare type ExtractAFromOption<O> = O extends Option<infer U> ? U : never;
/**
 * Type-level util to extract the type T in Array<T>
 */
export declare type ExtractTFromArray<A> = A extends Array<infer U> ? U : never;
/**
 * Type-level utils to extract the types L and A in Either<L, A>
 */
export declare type ExtractLFromEither<E> = E extends Either<infer U, unknown> ? U : never;
export declare type ExtractAFromEither<E> = E extends Either<unknown, infer U> ? U : never;
/**
 ...
 */
export declare type ObjectValues<A extends object> = A[keyof A];
/**
 ...
 * - WeakSet
 */
export declare type Values<T extends object> = T extends Record<string | number | symbol, infer A> ? A : T extends Array<infer B> ? B : T extends Map<unknown, infer C> ? C : T extends WeakMap<object, infer D> ? D : T extends Set<infer E> ? E : T extends WeakSet<infer F> ? F : unknown;
/**
 ...
 */
export declare type DistributiveKeyOf<T extends object> = T extends unknown ? keyof T : never;
/**
 ...
 */
export declare type DistributiveExact<A extends object, B extends A> = A extends unknown ? A & Partial<Record<Exclude<DistributiveKeyOf<B>, DistributiveKeyOf<A>>, never>> : never;
/**
 ...
 */
export declare type ExactRecordsUnion<A extends object> = DistributiveExact<A, A>;
/**
 * Select the member of a union type by matching on the the provided key, `K`,
 * and value, `V`. Returns `never` if no match occurs.
 */
export declare type SelectUnionMember<K extends PropertyKey, T, U extends object> = U extends {
    [P in K]: T;
} & infer A ? unknown extends A ? U : A : never;
/**
 ...
 */
export declare type SelectUnionMemberByTag<T extends string, U extends object> = SelectUnionMember<'tag', T, U>;
/**
 ...
 */
export declare type SelectUnionMemberByContents<C, U extends object> = SelectUnionMember<'contents', C, U>;
/**
 * A version of `Pick` that distributes over union types.
 */
export declare type DistributivePick<U extends object, K extends DistributiveKeyOf<U>> = {
    [P in K]: unknown extends U[P] ? {
        [P2 in P]: P2 extends keyof SelectUnionMember<P2, unknown, U> ? SelectUnionMember<P2, unknown, U>[P2] : never;
    }[P] : {
        [P2 in P]: U[P2];
    }[P];
};
 ...
 *          | { key: 'baz', value: string }
 */
export declare type ExtractKeyValuePairs<T, K extends keyof T = keyof T> = K extends keyof T ? {
    key: K;
    value: T[K];
} : never;
/**
 ...
 */
export declare type RaiseSubProperty<T, K extends keyof T, SK extends keyof T[K]> = Pick<T, Exclude<keyof T, K>> & {
    [k in K]: T[K][SK];
};
/**
