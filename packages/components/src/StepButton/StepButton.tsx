import { StepButton as MuiStepButton } from '@mui/material'

/**
 *
 * Demos:
 *
 * - [Step Button](https://simspace.github.io/monorail/main/storybook/?path=/docs/navigation-stepper-stepbutton--default)
 * - [Steppers (MUI)](https://mui.com/material-ui/react-stepper/)
 *
 * API:
 *
 * - [StepButton API](https://mui.com/material-ui/api/step-button/)
 * - inherits [ButtonBase API](https://mui.com/material-ui/api/button-base/)
 */
export const StepButton: typeof MuiStepButton = MuiStepButton

export * from '@mui/material/StepButton'
