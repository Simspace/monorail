import { ListItemIcon as MuiListItemIcon } from '@mui/material'

/**
 * A simple wrapper to apply `List` styles to an `Icon` or `SvgIcon`.
 *
 * Demos:
 *
 * - [List Item Icon](https://simspace.github.io/monorail/main/storybook/?path=/docs/data-display-list-listitemicon--default)
 * - [Lists (MUI)](https://mui.com/material-ui/react-list/)
 *
 * API:
 *
 * - [ListItemIcon API](https://mui.com/material-ui/api/list-item-icon/)
 */
export const ListItemIcon: typeof MuiListItemIcon = MuiListItemIcon

export * from '@mui/material/ListItemIcon'
