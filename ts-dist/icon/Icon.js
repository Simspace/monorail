...
import { Colors, getColor, convertAppNameToString, isAppName, } from '@monorail/helpers/exports';
...
export const Icon = styled(({ cssOverrides: _cssOverrides, icon, ...otherProps }) => {
    const CustomIcon = customIcons[icon] ||
        (isAppName(icon) && customIcons[`${convertAppNameToString(icon)}-app`]);
    if (CustomIcon) {
        return React.createElement(CustomIcon, Object.assign({}, otherProps));
    }
    return React.createElement("i", Object.assign({}, otherProps), icon);
