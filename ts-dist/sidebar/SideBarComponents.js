...
import { baseDisabledStyles, BorderRadius, borderRadius, Colors, getColor, ease, flexFlow, FontSizes, gothamFontFamily, Sizes, transition, typography, visible, } from '@monorail/helpers/exports';
import { ListItem, ListItemGraphic, ListItemPrimaryText, ListItemSecondaryText, ListItemText, } from '@monorail/list/List';
import posed from 'react-pose';
import { zIndex, ZIndexNodeName } from '@monorail/helpers/zIndex';
export const sideBarCollapsedTransitionDuration = 150;
export const sideBarCollapsedTransition = ({ properties = ['all'], isSideBarCollapsed }) => transition({
    properties,
    easing: ease(isSideBarCollapsed),
    duration: sideBarCollapsedTransitionDuration,
});
export var SidebarContainerAnimationPose;
...
})(SidebarContainerAnimationPose || (SidebarContainerAnimationPose = {}));
export const SidebarContainer = styled(posed.div({
...
`;
export const SidebarMenuContainer = styled.div(({ cssOverrides }) => css `
    ${flexFlow()};
    ...
    ${cssOverrides};
  `);
export const SidebarMenuItemDropDownToggle = styled(({ title, subtitle, iconName, isOpen, cssOverrides, isSideBarCollapsed, ...otherProps }) => (React.createElement(ListItem, Object.assign({ cssOverrides: css `
        ${borderRadius(BorderRadius.Medium)};
        ${cssOverrides};
      `, size: Sizes.DP40 }, otherProps),
    React.createElement(ListItemGraphic, { icon: iconName, cssOverrides: css `
          color: currentColor;
    ...
    React.createElement(ListItemText, null,
        React.createElement(ListItemPrimaryText, null, title),
        React.createElement(ListItemSecondaryText, { cssOverrides: isOpen &&
                css `
              color: ${getColor(Colors.AccentPurple700)};
            ` }, subtitle)),
    React.createElement(ListItemGraphic, { icon: isOpen ? 'arrow_drop_up' : 'arrow_drop_down', cssOverrides: css `
    ...
    ${cssOverrides};
  `);
export const SidebarMenuContextRibbon = styled.div `
