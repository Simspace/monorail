...
import { alphaCompare, numericCompare, ordAlpha, ordNumeric, ordRecordWithNameLower, recordWithNameLowerComparator, } from '../Ord';
describe('ordAlpha', () => {
...
describe('ordRecordWithNameLower', () => {
    it('should compare { name: string } records by lowercase alpha', () => {
        const actualA = ordRecordWithNameLower.compare({ name: 'Josh' }, { name: 'adam' });
        const expectedA = recordWithNameLowerComparator({ name: 'Josh' }, { name: 'adam' });
        expect(actualA).toBe(expectedA);
        expect(actualA).toBe(1);
        const actualB = ordRecordWithNameLower.compare({ name: 'emily' }, { name: 'Luke' });
        const expectedB = recordWithNameLowerComparator({ name: 'emily' }, { name: 'Luke' });
        expect(actualB).toBe(expectedB);
        expect(actualB).toBe(-1);
        const actualC = ordRecordWithNameLower.compare({ name: 'David' }, { name: 'david' });
        const expectedC = recordWithNameLowerComparator({ name: 'David' }, { name: 'david' });
        expect(actualC).toBe(expectedC);
        expect(actualC).toBe(0);
    });
});
//# sourceMappingURL=Ord.jest.js.map