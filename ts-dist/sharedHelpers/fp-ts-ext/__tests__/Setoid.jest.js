...
import { getSetoidShallowEq, getSetoidStrict, recordWithNameLowerEquality, setoidRecordWithNameLower, setoidShallowEq, setoidStrict, } from '../Setoid';
...
describe('setoidRecordWithNameLower', () => {
    it('should compare { name: string } records by lowercase alpha', () => {
        const actualA = setoidRecordWithNameLower.equals({ name: 'Josh B' }, { name: 'josh B' });
        const expectedA = recordWithNameLowerEquality({ name: 'Josh B' }, { name: 'josh B' });
        expect(actualA).toBe(expectedA);
        expect(actualA).toBe(true);
        const actualB = setoidRecordWithNameLower.equals({ name: 'Josh B' }, { name: 'joshua B' });
        const expectedB = recordWithNameLowerEquality({ name: 'Josh B' }, { name: 'joshua B' });
        expect(actualB).toBe(expectedB);
        expect(actualB).toBe(false);
    });
});
//# sourceMappingURL=Setoid.jest.js.map