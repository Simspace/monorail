...
const getDropAmounts = ({ dropXDirection, dropYDirection, innerHeight, innerWidth, ...otherProps }) => ({
...
const getDropXAmount = ({ dropXDirection, innerWidth, boundingRect, toSide, gap }) => {
    const isLeft = dropXDirection === 'left';
    const dropXAmountForToSide = () => isLeft
...
const getMaxHeight = ({ dropYDirection, innerHeight, boundingRect, toSide, gap }) => {
    const isTop = dropYDirection === 'top';
    const maxHeightForToSide = () => isTop
        ? innerHeight - boundingRect.top - gap * 2
        : boundingRect.top + boundingRect.height - gap * 2;
    const maxHeightForNotToSide = () => isTop
...
const getMaxWidth = ({ dropXDirection, innerWidth, boundingRect, toSide, gap }) => {
    const isLeft = dropXDirection === 'left';
    const maxWidthForToSide = () => isLeft
        ? innerWidth - boundingRect.left - boundingRect.width - gap * 2
        : boundingRect.left - gap * 2;
    const maxWidthForNotToSide = () => isLeft
...
const getMaxHeightCalc = ({ dropYDirection, innerHeight, boundingRect, toSide, gap }) => {
    const isTop = dropYDirection === 'top';
    const maxHeightForToSide = () => isTop
...
const getMaxWidthCalc = ({ dropXDirection, innerWidth, boundingRect, toSide, gap }) => {
    const isLeft = dropXDirection === 'left';
    const maxWidthForToSide = () => isLeft
...
export class PopOver extends Component {
    constructor() {
        ...
        this.onClick = (event) => {
            ...
            const dropYDirection = innerHeight / 2 > boundingRect.top + boundingRect.height / 2
                ? 'top'
                : 'bottom';
            const dropXDirection = innerWidth / 2 > boundingRect.left + boundingRect.width / 2
                ? 'left'
                : 'right';
            this.setState(({ isOpen }) => {
                this.props.onToggle && this.props.onToggle(!isOpen);
    ...
    render() {
        ...
        return (React.createElement(React.Fragment, null,
            ...
            shouldRender && (React.createElement(Portal, { document: document }, popOver({
                isOpen,
                position,
